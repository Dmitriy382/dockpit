...
✅ Тест подключения для каждого хоста

___
SH Туннелирование - Подробная инструкция
Как это работает?
Dockpit использует SSH туннель для безопасного подключения к удаленному Docker. Вот что происходит под капотом:

- Приложение создает SSH туннель: ssh -N -L local_port:/var/run/docker.sock user@remote_host
...
┌─────────────┐                ┌──────────────┐                ┌─────────────┐
│   Dockpit   │  localhost:X   │  SSH Tunnel  │    SSH (22)    │ Remote Host │
│  (Client)   │ ◄─────────────►│   (Secure)   │ ◄─────────────►│   Docker    │
└─────────────┘                └──────────────┘                └─────────────┘
---
...
# Проверка доступа
docker ps
# Команда должна работать БЕЗ sudo!

ВАЖНО: Если команда docker ps требует sudo, SSH туннель не будет работать! Обязательно добавьте пользователя в группу docker.
...
![Dockpit-ssh-connections-demo](./assets/ssh-connection-demo.gif)
---
